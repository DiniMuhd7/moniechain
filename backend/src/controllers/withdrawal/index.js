...
const submitWithdrawal = async (req, res) => {
  ...
  if (!Number.isFinite(numericAmount) || numericAmount <= 0 || !destination || !withdrawType || !reference || !provider) {
  ...
  const usdValue = (numericAmount * 0.01) / 1000;
  if (usdValue < 1) {
  ...
  try {
    ...
    const userDetails = await User.findByIdAndUpdate(
      { _id: userId, score: { $gte: numericAmount } },
      {
        ...
        $inc: { score: -numericAmount },
      },
      { new: true }
    );
    if (!userDetails) {
      return res.status(400).json({ success: false, message: "Insufficient WizPoints balance" });
    }

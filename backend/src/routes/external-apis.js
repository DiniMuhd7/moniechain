...
router.get("/countries", async (req, res) => {
  try {
    ...
    if (countryCache && Date.now() - countryLastFetchTime < CACHE_EXPIRATION) {
...
let languageLastFetchTime = null;
...
router.get("/languages", async (req, res) => {
  try {
    ...
    if (languageCache && Date.now() - languageLastFetchTime < CACHE_EXPIRATION) {
    ...
    languageLastFetchTime = Date.now();
...
let exchangeLastFetchTime = null;
...
router.get("/exchange-rate", async (req, res) => {
  try {
    // Check if the languages are cached and haven't expired
    if (exchangeCache && Date.now() - exchangeLastFetchTime < CACHE_EXPIRATION) {
    ...
    exchangeLastFetchTime = Date.now();
